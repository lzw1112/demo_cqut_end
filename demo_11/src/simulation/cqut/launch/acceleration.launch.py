...
def generate_launch_description():
    ...
    config_path = os.path.join(this_directory, "config", "acceleration.yaml")
    ...
    start_lidar_cluster_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory("lidar_cluster"), "launch", "lidar_cluster.launch.py")
        ),
    )

    start_line_detect_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory("line_detect"), "launch", "line_detect.launch.py")
        ),
    )

    start_planning_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory("path_generator"), "launch", "acceleration.launch.py")
        ),
    )

    start_control_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory("fsd_control"), "launch", "acceleration.launch.py")
        ),
    )
    ...
    ld.add_action(start_lidar_cluster_cmd)
    ld.add_action(start_line_detect_cmd)
