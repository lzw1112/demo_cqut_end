...
bool Autox_Track::genTraj() 
{
    ...
    // Find Center Line
    {
        ...
        {
            const auto it_blue = std::min_element( map_.cone_blue.begin(), map_.cone_blue.end(), [&](const cqut_msg::msg::Cone &a, const cqut_msg::msg::Cone &b) {
                const double da = std::hypot(red.position.x - a.position.x, red.position.y - a.position.y);
                const double db = std::hypot(red.position.x - b.position.x, red.position.y - b.position.y);
                return da < db;
            });
            cv::Point2f tmp;
...
bool Autox_Track::CalculateTraj(Trajectory &refline) 
{
    ...
    {
        RCLCPP_WARN(rclcpp::get_logger("Trackdrive_track"), "Trajectory is empty !");
        return false;
    }
    refline.clear();
    TrajectoryPoint tmp_pt;
