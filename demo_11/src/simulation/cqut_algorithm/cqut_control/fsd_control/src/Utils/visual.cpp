...
{
    double temp = (vel - control_param_.initial_velocity) / (control_param_.desire_vel - control_param_.initial_velocity);
    if (temp > 1)
        temp = 1;
    if (temp < 0)
        temp = 0;
...
void visual_map(const cqut_msg::msg::Map &map, visualization_msgs::msg::MarkerArray &visual) 
{
    ...
    for (size_t i = 0; i < map.cone_red.size(); i++) 
    {
        ...
        tmp.header.stamp = rclcpp::Clock().now();
        tmp.ns = "red_" + std::to_string(i);
        tmp.action = visualization_msgs::msg::Marker::ADD;
        tmp.type = visualization_msgs::msg::Marker::POINTS;
        tmp.scale.x = 0.8;
        tmp.scale.y = 0.8;
        tmp.scale.z = 0.8;
