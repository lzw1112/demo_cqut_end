...
{
// Constructor
LidarCluster::LidarCluster(rclcpp::Node::SharedPtr nh) : nh_(nh) 
{ 
    loadParameters(); 
};
...
// Getters
sensor_msgs::msg::PointCloud LidarCluster::getLidarCluster() 
{ 
    return cluster_; 
}

bool LidarCluster::is_ok() const 
{ 
    return is_ok_flag_; 
}

// Setters
void LidarCluster::setRawLidar(const sensor_msgs::msg::PointCloud2 &msg) 
{
    raw_pc2_ = msg;
    getRawLidar_ = true;
}
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_cones) 
{
    ...
    {
        if (std::hypot(iter.x, iter.y) < sqrt(2) || iter.z > 0.7 || iter.x < 0 || (std::hypot(iter.x, iter.y) > 7 && iter.z < 0.03))
          continue;
