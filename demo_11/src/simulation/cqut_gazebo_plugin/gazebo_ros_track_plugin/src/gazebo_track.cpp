...
class TrackStreamer : public gazebo::ModelPlugin 
{
public:
    enum SIDE 
    {
    ...
    geometry_msgs::msg::Point toGeomPoint(const boost::shared_ptr<gazebo::physics::Entity> &e) const 
    {
    ...
    void fillPoints(std::vector<geometry_msgs::msg::Point> &points,
    ...
    void fillHeader(visualization_msgs::msg::Marker &marker, const std::string &ns, int id, const rclcpp::Time &time) const 
    {
    ...
    void computeTKLine(const std::vector<boost::shared_ptr<gazebo::physics::Entity>> &tk_device_start,
                       const std::string &ns, const rclcpp::Time &time,
                       visualization_msgs::msg::MarkerArray &array) const 
    {
        ...
        marker.color.g            = 0.0;
        marker.color.b            = 1.0;
        marker.points             = std::vector<geometry_msgs::msg::Point>(2);
    ...
    {
        nh = gazebo_ros::Node::Get(_sdf, "track_streamer");

        RCLCPP_INFO(nh->get_logger(), "Loading Track");

        rclcpp::QoS qos(1);
        qos.durability(rclcpp::DurabilityPolicy::TransientLocal);
        pub_track = nh->create_publisher<visualization_msgs::msg::MarkerArray>(_sdf->Get<std::string>("markers") + "/markers", qos);
        pub_custome_track = nh->create_publisher<cqut_msg::msg::Track>(_sdf->Get<std::string>("markers"), qos);
        ...
        for (unsigned int i = 0; i < model->GetChildCount(); ++i) 
        {
            ...
            if (std::regex_match(name, std::regex("cone_left\\d+::link"))) 
            {
                left.push_back(entity);
            } 
            else if (std::regex_match(name, std::regex("cone_right\\d+::link"))) 
            {
                right.push_back(entity);
            } 
            else if (std::regex_match(name, std::regex("cone_orange\\d+::link"))) 
            {
                orange.push_back(entity);
            } 
            else if (std::regex_match(name, std::regex("cone_orange_big\\d+::link"))) 
            {
        ...
        const auto stamp = rclcpp::Time();
        {
            ...
            {
                addMarker(stamp, toPoint2d(right[i]), SIDE::RIGHT, "track", static_cast<int32_t>(left.size() + i), track_markers);
            }
            for (size_t i = 0; i < orange.size(); ++i) 
            {
                addMarker(stamp, toPoint2d(orange[i]), SIDE::ORANGE, "track", static_cast<int32_t>(left.size() + right.size() + i), track_markers);
            }
            for (size_t i = 0; i < orange_big.size(); ++i) 
            {
                addMarker(stamp, toPoint2d(orange_big[i]), SIDE::ORANGE_BIG, "track", static_cast<int32_t>(left.size() + right.size() + orange.size() + i), track_markers);
            }
            for (size_t i = 0; i < tk_device_start.size(); ++i) 
            {
                addMarker(stamp, toPoint2d(tk_device_start[i]), SIDE::TK_DEVICE, "track", static_cast<int32_t>(left.size() + right.size() + orange.size() + orange_big.size() + i), track_markers);
            }
            for (size_t i = 0; i < tk_device_end.size(); ++i) 
            {
                addMarker(stamp, toPoint2d(tk_device_end[i]), SIDE::TK_DEVICE, "track", static_cast<int32_t>(left.size() + right.size() + orange.size() + orange_big.size() + tk_device_start.size() + i), track_markers);
            }
        }
    ...
    void addMarker(const rclcpp::Time &time,
                   const Point2d &p,
                   const SIDE side,
                   const std::string &ns,
                   const int32_t id,
                   visualization_msgs::msg::MarkerArray &array) 
    {
        ...
        if (side == SIDE::LEFT) 
        {
            ...
            marker.color.g       = 0.0;
            marker.color.b       = 0.0;
            marker.mesh_resource = "package://cqut_description/meshes/cone/cone_blue.dae";
        ...
        else if (side == SIDE::RIGHT) 
        {
            ...
            marker.color.g       = 0.0;
            marker.color.b       = 1.0;
            marker.mesh_resource = "package://cqut_description/meshes/cone/cone_yellow.dae";
        ...
        else if (side == SIDE::ORANGE) 
        {
            ...
            marker.color.b       = 0.0;
            marker.mesh_resource = "package://cqut_description/meshes/cone/cone_orange.dae";
        } 
        else if (side == SIDE::ORANGE_BIG) 
        {
            ...
            marker.color.g       = static_cast<float>(165.0 / 255.0);
            marker.color.b       = 0.0;
            marker.mesh_resource = "package://cqut_description/meshes/cone/cone_orange_big.dae";
        } 
    ...
    rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr  pub_track;
    rclcpp::Publisher<cqut_msg::msg::Track>::SharedPtr  pub_custome_track;

};

GZ_REGISTER_MODEL_PLUGIN(TrackStreamer)

} // namespace gazebo_plusins
